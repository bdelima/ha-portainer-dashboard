...
5. On failure for a container that IS part of a stack: judgment is deferred rather than immediate. An earlier version tried to distinguish the known [network_mode:service conflict](#stack-restart-needed-flow-networkmodeservice-containers) from a genuine failure by matching the exception's text against Docker's own daemon message (`"conflicting options"` + `"network mode"`). **Confirmed broken in production** (see below) — HA core's own `portainer` integration wraps every `recreate_container` failure, whatever the underlying cause, into the same generic `HomeAssistantError`, so the real Docker/Portainer error text never reaches this code at all; the substring check was comparing against a message that could never contain it. It's been replaced with symptom-based detection: the container's `sensor.<name>_image` value is captured before the call, and on failure, `perform_update` watches that same entity for up to 150 seconds (long enough to span at least two of core's own 60-second Portainer-poll cycles) for what it actually settles on — a bare content digest (confirmed as the real, observable symptom) means the known conflict and triggers the stack-restart offer below; a normal-looking tag means the recreate apparently completed cleanly despite the exception; no change at all within the timeout means a genuine failure, and it's re-raised. See `_await_recreate_outcome` in `__init__.py`.
